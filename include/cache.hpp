...
class AddCache {
  ...
  public:
    AddCache(QubitCount q)
        : _tables{2}, rng(std::random_device()()), dist(0, 1) {
  ...
  private:
    ...
    struct alignas(
        hardware_constructive_interference_size) // the same cacheline
        Bucket {
        Entry e;
    };
...
class MulCache {
  ...
  public:
    MulCache(QubitCount q)
        : _tables{3}, rng(std::random_device()()), dist(0, 1) {
