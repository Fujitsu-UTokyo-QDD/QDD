...
class ComplexTable {
  ...
  public:
    struct Entry {
        ...
        ///
        /// The sign of number is encoded in the least significant bit of its
        /// entry pointer If not handled properly, this causes misaligned access
        ...
        [[nodiscard]] static inline Entry *flipPointerSign(const Entry *e) {
            if (e == &ComplexTable<NBUCKET, INITIAL_ALLOCATION_SIZE,
                                   GROWTH_FACTOR, INITIAL_GC_LIMIT>::zero) {
        ...
        [[nodiscard]] static constexpr bool
        approximatelyEquals(const fp left, const fp right) {
            // NOLINTNEXTLINE(clang-diagnostic-float-equal) equivalence check is
            // a shortcut before check with tolerance
        ...
        static void writeBinary(const Entry *e, std::ostream &os) {
            auto temp = val(e);
            os.write(reinterpret_cast<const char *>(&temp),
                     sizeof(decltype(temp)));
        }
    };
    ...
    ComplexTable() {
        // add 1/2 to the complex table and increase its ref count (so that it
        // is not collected)
    ...
    Entry *lookup(const fp &val) {
        ...
        // only the last entry in the lower bucket and the first entry in the
        // upper bucket need to be checked
        ...
        bool lowerMatchFound = (pLower != nullptr &&
                                Entry::approximatelyEquals(val, pLower->value));
        bool upperMatchFound = (pUpper != nullptr &&
                                Entry::approximatelyEquals(val, pUpper->value));
    ...
    std::size_t garbageCollect(bool force = false) {
        gcCalls++;
        // nothing to be done if garbage collection is not forced, and the limit
        // has not been reached, or the current count is minimal (the complex
        // table always contains at least 0.5)
        ...
        // The garbage collection limit changes dynamically depending on the
        // number of remaining (active) nodes. If it were not changed, garbage
        // collection would run through the complete table on each successive
        // call once the number of remaining entries reaches the garbage
        // collection limit. It is increased whenever the number of remaining
        // entries is rather close to the garbage collection threshold and
        // decreased if the number of remaining entries is much lower than the
        // current limit.
    ...
    void clear() {
        ...
        // release memory of all but the first chunk TODO: it could be desirable
        // to keep the memory
        while (chunkID > 0) {
            chunks.pop_back();
            chunkID--;
    ...
    void print() {
        ...
        std::cout.precision(std::numeric_limits<fp>::max_digits10);
        for (std::size_t key = 0; key < table.size(); ++key) {
            auto p = table[key];
            if (p != nullptr) {
                std::cout << key << ": "
    ...
    [[nodiscard]] fp hitRatio() const {
        return static_cast<fp>(hits) / lookups;
    }
    ...
    inline Entry *findOrInsert(const std::int64_t key, const fp val) {
        ...
        while (curr != nullptr && curr->value <= valTol) {
            if (Entry::approximatelyEquals(curr->value, val)) {
                // check if val is actually closer to the next element in the
...
struct Complex {
    ...
    auto getLowestFraction(const double x,
                           const std::uint64_t maxDenominator = 1U << 10,
                           const fp tol = ComplexTable<>::tolerance()) const {
        assert(x >= 0.);
    ...
    void printFormatted(std::ostream &os, fp num,
                        bool imaginary = false) const {
        if (std::abs(num) <= ComplexTable<>::tolerance()) {
            os << (std::signbit(num) ? "-" : "+") << "0"
               << (imaginary ? "i" : "");
            return;
        }
        ...
        if (error <=
            ComplexTable<>::tolerance()) { // suitable fraction a/b found
            const std::string sign =
                std::signbit(num) ? "-" : (imaginary ? "+" : "");
        ...
        if (error <=
            ComplexTable<>::tolerance()) { // suitable fraction a/b π found
            const std::string sign =
                std::signbit(num) ? "-" : (imaginary ? "+" : "");
    ...
    std::string toString(const fp &real, const fp &imag, bool formatted = true,
    ...
    std::string toString(bool formatted = true, int precision = -1) const {
        return toString(CTEntry::val(this->r), CTEntry::val(this->i), formatted,
                        precision);
    };
};
...
template <> struct hash<Complex> {
    std::size_t operator()(Complex const &c) const noexcept {
        auto h1 = murmur_hash(reinterpret_cast<std::size_t>(c.r));
        auto h2 = murmur_hash(reinterpret_cast<std::size_t>(c.i));
...
class ComplexCache {
  ...
  public:
    ...
    void clear() {
        ...
        // release memory of all but the first chunk TODO: it could be
        // desirable to keep the memory
        while (chunkID > 0) {
            chunks.pop_back();
            chunkID--;
        }
