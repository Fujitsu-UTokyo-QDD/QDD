      ...
      for (int b0 = 0; b0 < 2; b0++) {
        ...
        if (it != c.end() && it->qubit == z) {
          if (it->type == Control::Type::neg)
            edges[i] =
                makeMEdge(z, {edges[i], mEdge::zero, mEdge::zero,
          ...
          edges[i] =
              makeMEdge(z, {edges[i], mEdge::zero, mEdge::zero, edges[i]});
        }
      }
    }
...
char measureOneCollapsing(vEdge &rootEdge, const Qubit index,
  ...
  double normalizationFactor; // NOLINT(cppcoreguidelines-init-variables) always
...
GateMatrix u3(float theta, float phi, float lambda) {
  ...
  std::complex<float> i2 =
      -std::exp(std::complex<float>(0, lambda)) * std::sin(theta / 2);
  std::complex<float> i3 =
      std::exp(std::complex<float>(0, phi)) * std::sin(theta / 2);
  std::complex<float> i4 =
      std::exp(std::complex<float>(0, lambda + phi)) * std::cos(theta / 2);
...
GateMatrix r(float theta, float phi) {
  std::complex<float> i1 = {std::cos(theta / 2), 0};
  std::complex<float> i2 = std::complex<float>(0, -1) *
                           std::exp(std::complex<float>(0, -phi)) *
                           std::sin(theta / 2);
  std::complex<float> i3 = std::complex<float>(0, -1) *
                           std::exp(std::complex<float>(0, phi)) *
                           std::sin(theta / 2);
  return GateMatrix{i1, i2, i3, i1};
}
...
inline long max_leaf_nodes(Qubit root) {
  QubitCount qc = root + 1;
  return (1 << qc) * (1 << qc);
}
inline long max_leaf_nodes(QubitCount qc) { return (1 << qc) * (1 << qc); }

static mNode *makeFullDD(Qubit root, std::function<double()> wdist) {
  if (root == 0) {
    std::array<mEdge, 4> children = {
        {{Complex(wdist(), wdist()), mNode::terminal},
         {Complex(wdist(), wdist()), mNode::terminal},
         {Complex(wdist(), wdist()), mNode::terminal},
         {Complex(wdist(), wdist()), mNode::terminal}}};
    mNode *n = new mNode(root, children, nullptr);
    return n;
  }

  std::array<mEdge, 4> children = {
      {{Complex(wdist(), wdist()), makeFullDD(root - 1, wdist)},
       {Complex(wdist(), wdist()), makeFullDD(root - 1, wdist)},
       {Complex(wdist(), wdist()), makeFullDD(root - 1, wdist)},
       {Complex(wdist(), wdist()), makeFullDD(root - 1, wdist)}}};
  mNode *n = new mNode(root, children, nullptr);
...
static mNode *_randomDD(Qubit root, long count, std::function<double()> wdist) {
  if (count == 0) {
    return mNode::terminal;
  }
  long max_nodes = max_leaf_nodes(root);
  // std::cout << "root: " << root << ", count: " << count
  //          << ", max count: " << max_nodes << std::endl;
  if (max_nodes <= count) {
    // std::cout << "    make max" << std::endl;
    return makeFullDD(root, wdist);
  } else if (count <= 4) {
    ...
    std::array<mEdge, 4> children = {
        {{Complex(wdist(), wdist()), mNode::terminal},
         ...
         {Complex(wdist(), wdist()), mNode::terminal},
         {Complex(wdist(), wdist()), mNode::terminal}}};
    return new mNode(root, children, nullptr);
  } else {
    assert(root > 0);
    long next_max_nodes = max_leaf_nodes(root - 1);
    long c0 = std::min(next_max_nodes, count);
    count = std::max(count - next_max_nodes, 0l);
    long c1 = std::min(next_max_nodes, count);
    count = std::max(count - next_max_nodes, 0l);
    long c2 = std::min(next_max_nodes, count);
    count = std::max(count - next_max_nodes, 0l);
    long c3 = std::min(next_max_nodes, count);
    count = std::max(count - next_max_nodes, 0l);
    // std::cout << "c0 = " << c0 << ", c1 = " << c1 << ", c2 = " << c2
    //<< ", c3 = " << c3 << std::endl;
    std::array<mEdge, 4> children = {
        {{Complex(wdist(), wdist()), _randomDD(root - 1, c0, wdist)},
         {Complex(wdist(), wdist()), _randomDD(root - 1, c1, wdist)},
         {Complex(wdist(), wdist()), _randomDD(root - 1, c2, wdist)},
         {Complex(wdist(), wdist()), _randomDD(root - 1, c3, wdist)}}};
    return new mNode(root, children, nullptr);
  }
}

mEdge randomDD(QubitCount qc, float ratio) {
  if (ratio < 0.0 || ratio > 1) {
    return mEdge::zero;
  }
  long max_nodes = max_leaf_nodes(qc);
  long required_nodes = std::floor((float)max_nodes * ratio);
  std::cout << "max: " << max_nodes << ", required: " << required_nodes
            << std::endl;
  std::random_device rd;
  std::mt19937 gen(rd());
  std::uniform_real_distribution<double> dis(-1.0, 1.0);
  std::function<double()> wdist = [&]() { return dis(gen); };
...
long countNode(const mEdge &e) {
  if (e.isTerminal())
    return 1;
  mNode *n = e.getNode();
  return countNode(n->getEdge(0)) + countNode(n->getEdge(1)) +
         countNode(n->getEdge(2)) + countNode(n->getEdge(3));
}
