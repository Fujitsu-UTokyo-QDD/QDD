...
#pragma omp taskloop collapse(2) num_tasks(8) default(shared) private(x, y)
  ...
  if (current_var > LIMIT) {
    ...
    for (int i = 0; i < 8; i += 2) {
      edges[i / 2] = mm_add_fiber2(products[i].get(), products[i + 1].get(),
                                   current_var - 1);
    }
  } else {
    for (int i = 0; i < 8; i += 2) {
      edges[i / 2] = mm_add_fiber2(products_wo_future[i],
                                   products_wo_future[i + 1], current_var - 1);
    }
  }
...
#pragma omp taskloop collapse(2) num_tasks(4) default(shared) private(x, y)
  ...
  if (current_var > LIMIT) {
    ...
    edges[0] =
        vv_add_fiber2(products[0].get(), products[1].get(), current_var - 1);
    edges[1] =
        vv_add_fiber2(products[2].get(), products[3].get(), current_var - 1);
  } else {
...
mEdge H(QubitCount qnum, int target) { return makeGate(qnum, Hmat, target); }
mEdge S(QubitCount qnum, int target) { return makeGate(qnum, Smat, target); }
mEdge T(QubitCount qnum, int target) {
  std::complex<float> i1 = {1, 0};
  std::complex<float> i2 = {0, 0};
  std::complex<float> i3 = cf_1plusi;

  return makeGate(qnum, GateMatrix{i1, i2, i2, i3}, target);
}
