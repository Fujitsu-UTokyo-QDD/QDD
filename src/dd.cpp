...
vEdge gc(vEdge state) {
  ...
  std::cout << "vSize=" << vUnique.get_allocations()
            << " mSize=" << mUnique.get_allocations() << " vLimit=" << GC_SIZE;
...
mEdge conjugateTransposeRec(const mEdge &a) {
  if (a.isTerminal()) { // terminal case
    return {Complex(a.w.r, -a.w.i), a.n};
  }

  std::array<mEdge, 4> e{};
  for (auto i = 0U; i < 2; ++i) {
    for (auto j = 0U; j < 2; ++j) {
      e[2 * i + j] = conjugateTransposeRec(a.n->children[2 * j + i]);
    }
  }
  auto res = makeMEdge(a.n->v, e);

  // adjust top weight including conjugate
  return {res.w * Complex(a.w.r, -a.w.i), res.n};
}
