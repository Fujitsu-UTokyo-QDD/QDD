...
TEST(QddTest, GateTest) {
  ...
  {
    mEdge m =
        makeGate(2, Xmat, 0, Controls{Control{(Qubit)1, Control::Type::pos}});
    size_t dim;
    std_complex **mat = m.getMatrix(&dim);
    ASSERT_TRUE(mat[0][0] == (std::complex<double>(1.0, 0.0)));
    ASSERT_TRUE(mat[0][1] == (std::complex<double>(0.0, 0.0)));
  ...
  }
  {
    ...
    ASSERT_TRUE(mat[1][0] == std::complex<double>(0.0, 0.0));
    ASSERT_TRUE(
        isNearlyEqual(mat[1][1], std::exp(std::complex<double>(0, PI / 2))));
  }
  {
  ...
  }
  {
    ...
    ASSERT_TRUE(mat[1][0] == std::complex<double>(0.0, 0.0));
    ASSERT_TRUE(
        isNearlyEqual(mat[1][1], std::exp(std::complex<double>(0, PI / 4))));
  }
  {
  ...
  }
  {
    for (int i = 0; i < 16; i++) {
      ...
      size_t dim;
      std_complex **mat = m.getMatrix(&dim);
      ASSERT_TRUE(isNearlyEqual(mat[0][0],
