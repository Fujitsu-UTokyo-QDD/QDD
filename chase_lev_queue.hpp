...
namespace detail {
    ...
    template <Simple T> struct RingBuff {
      ...
      private:
        ...
        std::unique_ptr<T[]> _buff = std::make_unique<T[]>(_cap);
    };
...
template <Simple T> class CLQueue{
  public:
    // Constructs the deque with a given capacity the capacity of the deque (must be power of 2)
    explicit CLQueue(std::int64_t cap = 1024);

    // Move/Copy is not supported
    CLQueue(CLQueue const& other) = delete;
    CLQueue& operator=(CLQueue const& other) = delete;
...
template <Simple T> CLQueue<T>::CLQueue(std::int64_t cap)
    : _top(0), _bottom(0), _buffer(new detail::RingBuff<T>{cap}) {
    _garbage.reserve(32);
}

template <Simple T> std::size_t CLQueue<T>::size() const noexcept {
    int64_t b = _bottom.load(relaxed);
    int64_t t = _top.load(relaxed);
    return static_cast<std::size_t>(b >= t ? b - t : 0);
}

template <Simple T> int64_t CLQueue<T>::capacity() const noexcept {
    return _buffer.load(relaxed)->capacity();
}

template <Simple T> bool CLQueue<T>::empty() const noexcept { return !size(); }

template <Simple T> template <typename... Args> void CLQueue<T>::emplace(Args&&... args) {
...
template <Simple T> std::optional<T> CLQueue<T>::pop() noexcept {
...
template <Simple T> std::optional<T> CLQueue<T>::steal() noexcept {
    std::int64_t t = _top.load(acquire);
    std::atomic_thread_fence(seq_cst);
...
template <Simple T> CLQueue<T>::~CLQueue() noexcept { delete _buffer.load(); }
